...
class PostgresCustomizedRepositoryContextImplTest {
  ...
  @InjectMocks
  PostgresqlCustomizedRepositoryContextImpl customizedRepositoryContext;
  ...
  private void then_assertUpdateExecuted(int expectedCount) {
    ...
    verify(entityManager).createNativeQuery("set local lock_timeout = 3000");
    verify(query).executeUpdate();
  }

  private void then_assertFetchedTimeout(long expectedTimeout) {
    assertThat(timeout).isEqualTo(expectedTimeout);
    verify(entityManager).createNativeQuery("show lock_timeout");
