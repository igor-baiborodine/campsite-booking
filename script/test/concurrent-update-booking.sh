...
response=$(curl -X POST -H "Content-Type: application/json" -d "$payload" "$base_url"/api/v2/booking)
printf "Response: %s\n" "$response"
...
curl -X PUT -H "Content-Type: application/json" -d "$payload1" "$base_url"/api/v2/booking/"$uuid" & \
  curl -X PUT -H "Content-Type: application/json" -d "$payload2" "$base_url"/api/v2/booking/"$uuid"
