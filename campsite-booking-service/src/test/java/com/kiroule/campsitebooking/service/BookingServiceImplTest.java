...
import com.kiroule.campsitebooking.DisplayNamePrefix;
...
class BookingServiceImplTest {
  ...
  class CreateBooking {
    ...
    @Test
    void happy_path() {
      Booking booking = TestDataHelper.nextBooking().toBuilder().uuid(null).version(null).build();
      BookingEntity bookingEntity = TestDataHelper.nextBookingEntity();
    ...
    void given_booking_dates_not_available__then_BookingDatesNotAvailableException_thrown() {
      ...
      Booking existingBooking =
          TestDataHelper.nextBooking().toBuilder()
              .startDate(booking.getStartDate())
              .endDate(booking.getEndDate())
              .build();
      BookingEntity existingBookingEntity = TestDataHelper.nextBookingEntity();
    ...
    void given_new_booking_not_active__then_IllegalArgumentException_thrown() {
      // given
      Booking booking = TestDataHelper.nextBooking().toBuilder().active(false).build();
      // when
      Executable executable = () -> classUnderTest.createBooking(booking);
      // then
      assertThrows(IllegalArgumentException.class, executable);
  ...
  class FindVacantDates {
    ...
    void given_booking_dates_overlap_range_dates__then_no_vacant_dates_found() {
      ...
      Booking existingBooking =
          TestDataHelper.nextBooking().toBuilder().startDate(existingStartDate).endDate(existingEndDate).build();
      BookingEntity existingBookingEntity =
          TestDataHelper.nextBookingEntity().toBuilder()
    ...
    void given_booking_dates_same_as_range_dates__then_end_date_found() {
      ...
      Booking existingBooking =
          TestDataHelper.nextBooking().toBuilder().startDate(startDate).endDate(endDate).build();
      BookingEntity existingBookingEntity =
          TestDataHelper.nextBookingEntity().toBuilder().startDate(startDate).endDate(endDate).build();
      doReturn(singletonList(existingBookingEntity))
          .when(bookingRepository)
          .findForDateRange(any(), any(), any());
      doReturn(singletonList(existingBooking)).when(bookingMapper).toBookingsList(any());
